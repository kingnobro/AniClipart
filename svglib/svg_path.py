...
class SVGPath:
    ...
    def from_xml(x: minidom.Element):
        ...
        s = x.getAttribute('d')
        return SVGPath.from_str(s, fill=fill, filling=filling, color=color)

    @staticmethod
    def from_str(s: str, fill=False, filling=Filling.OUTLINE, color=None, add_closing=False):
        path_commands = []
        ...
        return SVGPath.from_commands(path_commands, fill=fill, filling=filling, color=color, add_closing=add_closing)
    ...
    @staticmethod
    def from_commands(path_commands: List[SVGCommand], fill=False, filling=Filling.OUTLINE, color=None, add_closing=False, allow_empty=False):
        from .svg_primitive import SVGPathGroup
        ...
        if svg_path is not None and (allow_empty or svg_path.path_commands):  # SVGPath contains at least one command
            ...
            if not svg_path.path_commands:
                svg_path.path_commands.append(empty_command)
            svg_paths.append(svg_path)
        return SVGPathGroup(svg_paths, fill=fill, color=color)
